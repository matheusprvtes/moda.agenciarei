...
A LP é **clara**. `styles.css` traz a estrutura e os tokens escuros do sistema
da marca; `theme-light.css` sobrescreve as cores para o fundo branco e é carregado
em todas as páginas, que trazem `data-theme="light"` na tag `<html>`.

Manter os dois arquivos é proposital: as regras de layout, tipografia e animação
ficam num lugar só e o tema é uma camada fina por cima (~5 KB). Para voltar ao
escuro, basta remover o atributo `data-theme` — nada mais precisa mudar.
